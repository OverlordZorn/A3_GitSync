...
if (count (nearestLocations [_pos, vn_ron_interruption_location_types, vn_ron_interruption_village_distance]) != 0) then { _canRON = false; systemChat "You're too close to a village" };
...
if ( count ( [_pos, vn_ron_interruption_enemy_sides, vn_ron_interruption_enemy_distance ] call vn_ron_fnc_nearbyUnits ) ) then {
    _canRON = false;
